...
def show_kpis(df, df_total, df_gender):
    ...
    cards = [
        ...
        ("#e0ae27",    "⚠️ Highest Risk Country", top_country,           f"{top_val:.2f}"),
        ("#5A6480",    "⚖️ Gender Gap",           f"{gender_gap:.2f}",   "Male − Female"),
    ...
    for col, (color, label, value, sub) in zip(cols, cards):
        col.markdown(f"""
        <div style="background:rgba(86, 45, 137, 0.15);border:1px solid #D0D8F0;border-radius:12px;padding:16px 18px";rgba(249,155,255, 0.25)>
          <div style="color:#8899aa;font-size:10px;font-weight:600;letter-spacing:1px;text-transform:uppercase;margin-bottom:8px">{label}</div>
...
def show_trend_kpis(df_total):
    ...
    c1, c2 = st.columns(2)
    c1.markdown(f"""
    <div style="background:##000000;border:1px solid #1e2d42;border-radius:12px;padding:16px 18px">
      <div style="color:#8899aa;font-size:10px;font-weight:600;letter-spacing:1px;text-transform:uppercase;margin-bottom:8px">📅 Best Performing Year</div>
    ...
    c2.markdown(f"""
    <div style="background:##000000;border:1px solid #1e2d42;border-radius:12px;padding:16px 18px">
      <div style="color:#8899aa;font-size:10px;font-weight:600;letter-spacing:1px;text-transform:uppercase;margin-bottom:8px">🏆 Best Performing Country</div>
